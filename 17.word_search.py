...
class Solution_1:
    ...
    def search(self, board, y, x, ind, word):
        if ind == len(word):
            return True
        if y < 0 or y >= len(board) or x < 0 or x >= len(board[y]) or board[y][x] != word[ind]:
            return False
        
        temp = board[y][x]
        board[y][x] = ' '
        found = self.search(board, y + 1, x, ind + 1, word) or self.search(board, y - 1, x, ind + 1, word ) or self.search(board, y, x + 1, ind + 1, word ) or self.search(board, y, x - 1, ind + 1, word )
